...
export async function createUser(token: string, user: ICreateUserWithIPlanInterface): Promise<IUserInterface> {
  try {
...
export async function findUsers(token: string, query: IUserFilter): Promise<IUserInterface[]> {
  try {    
    const response = await api.get<IUserInterface[]>(`/user?userTypeId=${query.userTypeId}&searchName=${query.searchName}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    return response.data;
  } catch (error) {
...
export async function findCurrentUser(token: string): Promise<IUserInterface | null> {
  try {
...
export async function updateUser(token: string, id: string, user: IUpdateUserWithIPlanInterface): Promise<IUserInterface> {
  try {
